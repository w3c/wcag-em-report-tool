...
describe('Controller: AuditSamplePagesCtrl', function () {
    ...
    beforeEach(inject(function($controller, $rootScope, _evalModel_) {
        ...
        expect(sampleModel.randomSample.webpage.length).toBe(0);
        expect(sampleModel.structuredSample.webpage.length).toBe(0);
    }));
    ...
    it('has access to filled pages', function () {
        ...
        filledPages = scope.filledPages();
        expect(filledPages[2]).toBe(page2);
        expect(filledPages[3]).toBe(page3);
        expect(filledPages.length).toBe(4);
    ...
    it('complete and uncomplete selected pages', function () {
        ...
        scope.completePages();
        expect(page0.tested).toBe(true);
        expect(page1.tested).toBe(true);

        page0.selected = false;
        scope.uncompletePages();
        expect(page0.tested).toBe(true);
        expect(page1.tested).toBe(false);

        page0.selected = true;
        page1.selected = false;
        scope.uncompletePages();
        expect(page0.tested).toBe(false);
        expect(page1.tested).toBe(false);
